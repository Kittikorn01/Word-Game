...
export function createStageCompleteOverlay(host: HTMLElement, content: StageCompleteContent, onNext: () => void) {
  ...
  button.hidden = !content.hasNextStage;
  element.tabIndex = -1;
  const abort = new AbortController();
  button.addEventListener('click', () => { if (button.disabled) return; button.disabled = true; onNext(); }, { signal: abort.signal });
  element.addEventListener('keydown', event => {
    if (event.key === 'Tab') { event.preventDefault(); (content.hasNextStage ? button : element).focus(); }
  }, { signal: abort.signal });
  host.append(element); (content.hasNextStage ? button : element).focus();
